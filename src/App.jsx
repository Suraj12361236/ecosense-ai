/**
 * @fileoverview EcoSense AI — main application component.
 * Re-exports all constants and pure functions from their respective modules
 ...
 *  - ErrorBoundary.jsx : React error boundary (used in main.jsx)
 *  - App.jsx           : UI components and application shell (this file)
 */
...
import {
  ...
  Save, Droplets, ShoppingBag, ChevronRight, ChevronLeft,
  CheckCircle2, Star, Download, Lightbulb, Crown, Menu,
  Loader2, Plus, ArrowRight, Info, Users, Wind,
} from "lucide-react";
import {
...
export {
  ...
  DEFAULT_INPUTS, XP_PER_LEVEL,
} from "./constants.js";

export {
  computeFootprint, levelFromXP, computeSustainabilityScore,
  computeTreesNeeded, computePredictions, buildRoadmap,
...
import {
  ...
  DIET_FACTORS, WASTE_FACTORS, WATER_FACTOR, SHOPPING_FACTOR,
  NATIONAL_AVG, GLOBAL_AVG, PARIS_TARGET, TREE_ABSORPTION,
  DEFAULT_INPUTS, XP_PER_LEVEL, DAILY_MISSIONS, ECO_TIPS, GREEN_PRODUCTS,
} from "./constants.js";
...
/* ============================================================
   LOCAL CONSTANTS (UI-only, not exported)
   ============================================================ */
const CATEGORY_META = {
...
/* ============================================================
   ...
   ============================================================ */
/**
 * Sends a message to the AI proxy endpoint.
...
/** @returns {Object} Recharts tooltip style matching current theme */
 ...
 */
function Card({ children, className = "", style, as: Tag = "div", ...rest }) {
  return <Tag className={`glass-card ${className}`} style={style} {...rest}>{children}</Tag>;
}
Card.propTypes = {
 ...
 * @param {Object} props
 * @param {React.ReactNode} props.children
 * @param {"default"|"warn"|"danger"} [props.tone]
 */
...
Pill.propTypes = {
  children: PropTypes.node.isRequired,
  tone:     PropTypes.oneOf(["default", "warn", "danger"]),
};
...
function RadialScore({ value, max = 100, label, sublabel, size = 140 }) {
  ...
  return (
    <div className="radial-wrap" style={{ width: size, height: size }} role="img" aria-label={`${label}: ${Math.round(value)} out of ${max}`}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden="true">
        <circle cx={size/2} cy={size/2} r={r} stroke="var(--surface-border)" strokeWidth="10" fill="none" />
        <circle cx={size/2} cy={size/2} r={r} stroke="url(#rg)" strokeWidth="10" fill="none"
...
/**
 * Semi-circular carbon footprint gauge (SVG).
 * @param {Object} props
 * @param {number} props.totalTons - Annual footprint in tons CO₂e
 */
function FootprintGauge({ totalTons }) {
  ...
  /** @param {number} from @param {number} to @returns {string} */
  ...
  return (
    <svg viewBox="0 0 300 170" className="gauge-svg" role="img"
      aria-label={`Carbon footprint: ${totalTons.toFixed(2)} tons CO2e per year. Green = below ${PARIS_TARGET}t target. Yellow = moderate. Red = high.`}>
      <title>Carbon Footprint Gauge</title>
      {zones.map((z, i) => <path key={i} d={arcPath(z.from, z.to)} stroke={z.color} strokeWidth="16" fill="none" aria-hidden="true" />)}
...
FootprintGauge.propTypes = { totalTons: PropTypes.number.isRequired };

/**
 * Accessible form row with label and optional hint.
 * @param {Object} props
 * @param {string} props.label
...
function NumInput({ id, value, onChange, min = 0, step = 1, ariaDescribedBy }) {
  return (
    <input id={id} type="number" min={min} step={step} value={value}
      onChange={(e) => onChange(Number(e.target.value))}
      aria-describedby={ariaDescribedBy} className="form-input" />
  );
}
NumInput.propTypes = {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
        ...
        <Card>
          ...
          <div className="gauge-card">
            ...
            <dl className="compare-list">
              {[["Sustainability score",`${score} / 100`],["1.5°C-aligned target",`${PARIS_TARGET.toFixed(1)} t`],["India average",`${NATIONAL_AVG.toFixed(1)} t`],["Global average",`${GLOBAL_AVG.toFixed(1)} t`]].map(([k,v])=>(
      ...
      <Card style={{ marginTop: 20 }}>
        ...
        {trendData.length === 0
          ? <EmptyState icon={TrendingUp} title="No history yet" text="Save a calculation to start building your trend." />
...
DashboardPage.propTypes = {
  ...
  gami:         PropTypes.object.isRequired,
        ...
        <Card>
          {step === 0 && (
            <fieldset className="fade-in">
              ...
              <FormRow label="Car / taxi distance (km)" htmlFor="carKm"><NumInput id="carKm" value={inputs.carKm} onChange={(v) => update("carKm", v)} /></FormRow>
              <FormRow label="Car fuel type" htmlFor="carType"><SelectInput id="carType" value={inputs.carType} onChange={(v) => update("carType", v)} options={[{value:"petrol",label:"Petrol"},{value:"diesel",label:"Diesel"},{value:"cng",label:"CNG"},{value:"ev",label:"Electric"}]} /></FormRow>
              <FormRow label="Two-wheeler distance (km)" htmlFor="twoWheelerKm"><NumInput id="twoWheelerKm" value={inputs.twoWheelerKm} onChange={(v) => update("twoWheelerKm", v)} /></FormRow>
              <FormRow label="Bus / train distance (km)" htmlFor="publicKm"><NumInput id="publicKm" value={inputs.publicKm} onChange={(v) => update("publicKm", v)} /></FormRow>
              <FormRow label="Domestic flights (round trips/yr)" htmlFor="domesticFlights"><NumInput id="domesticFlights" value={inputs.domesticFlights} onChange={(v) => update("domesticFlights", v)} /></FormRow>
              <FormRow label="International flights (round trips/yr)" htmlFor="intlFlights"><NumInput id="intlFlights" value={inputs.intlFlights} onChange={(v) => update("intlFlights", v)} /></FormRow>
            </fieldset>
          )}
          {step === 1 && (
            <fieldset className="fade-in">
              <legend className="card-title"><span aria-hidden="true"><Zap size={18} /></span> Home energy — per month</legend>
              <FormRow label="Electricity use (kWh)" htmlFor="electricity"><NumInput id="electricity" value={inputs.electricity} onChange={(v) => update("electricity", v)} /></FormRow>
              <FormRow label="LPG cylinders (14.2 kg)" htmlFor="lpgCylinders"><NumInput id="lpgCylinders" value={inputs.lpgCylinders} onChange={(v) => update("lpgCylinders", v)} step={0.5} /></FormRow>
              <p className="hint-text"><Info size={13} aria-hidden="true" /> Tip: use the Bill Scanner to auto-fill your electricity number.</p>
            </fieldset>
          )}
          {step === 2 && (
            <fieldset className="fade-in">
              <legend className="card-title"><span aria-hidden="true"><Utensils size={18} /></span> Food & diet</legend>
              <FormRow label="Diet pattern" htmlFor="diet"><SelectInput id="diet" value={inputs.diet} onChange={(v) => update("diet", v)} options={[{value:"vegan",label:"Vegan"},{value:"vegetarian",label:"Vegetarian"},{value:"eggetarian",label:"Eggetarian"},{value:"moderate",label:"Non-veg, moderate"},{value:"heavy",label:"Non-veg, daily"}]} /></FormRow>
            </fieldset>
          )}
          {step === 3 && (
            <fieldset className="fade-in">
              <legend className="card-title"><span aria-hidden="true"><Droplets size={18} /></span> Water & waste</legend>
              <FormRow label="Water use (litres/day)" htmlFor="waterLpd"><NumInput id="waterLpd" value={inputs.waterLpd} onChange={(v) => update("waterLpd", v)} /></FormRow>
              <FormRow label="Waste segregation habit" htmlFor="waste"><SelectInput id="waste" value={inputs.waste} onChange={(v) => update("waste", v)} options={[{value:"most",label:"Segregate most waste"},{value:"sometimes",label:"Sometimes"},{value:"rarely",label:"Rarely"}]} /></FormRow>
            </fieldset>
          )}
          {step === 4 && (
            <fieldset className="fade-in">
              <legend className="card-title"><span aria-hidden="true"><ShoppingBag size={18} /></span> Shopping & lifestyle</legend>
              <FormRow label="Monthly discretionary shopping (₹)" htmlFor="shoppingSpend"><NumInput id="shoppingSpend" value={inputs.shoppingSpend} onChange={(v) => update("shoppingSpend", v)} step={100} /></FormRow>
            </fieldset>
          )}
          {step === 5 && (
            <div className="fade-in">
              ...
              <div className="footer-row">
                <button className="btn btn-primary" onClick={onSave} aria-label="Save footprint to log"><Save size={15} aria-hidden="true" /> Save to log</button>
                {saveStatus && <span className="status-text" role="status" aria-live="polite">{saveStatus}</span>}
              </div>
            </div>
          )}
          <div className="wizard-nav">
            <button className="btn btn-ghost" disabled={step===0} onClick={()=>setStep(s=>Math.max(0,s-1))} aria-label="Previous step"><ChevronLeft size={15} aria-hidden="true" /> Back</button>
            <button className="btn btn-primary" disabled={step===WIZARD_STEPS.length-1} onClick={()=>setStep(s=>Math.min(WIZARD_STEPS.length-1,s+1))} aria-label="Next step">Next <ChevronRight size={15} aria-hidden="true" /></button>
          </div>
        </Card>
        <Card>
          <h2 className="card-title"><span aria-hidden="true"><Sparkles size={18} /></span> Live summary</h2>
          <p className="live-total" aria-live="polite" aria-atomic="true">{result.totalTons.toFixed(2)} <span>t CO2e / year</span></p>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
...
/* ============================================================
   AI COACH PAGE
   ============================================================ */
function CoachPage({ result }) {
  const [messages, setMessages] = useState([{ role: "assistant", content: "Hi! I'm your EcoSense AI climate coach. Tell me about your daily routine, or tap \"Generate my plan\" to analyse your calculator data." }]);
  ...
  const systemPrompt = `You are the EcoSense AI climate coach. The user's footprint is ${result.totalTons.toFixed(2)} t CO₂e/year. Breakdown: ${Object.entries(result.breakdown).map(([k,v])=>`${k}: ${v.toFixed(2)}t`).join(", ")}. India avg ${NATIONAL_AVG}t, global avg ${GLOBAL_AVG}t, 1.5°C target ${PARIS_TARGET}t. Give: impact summary, 2-3 recommendations, monthly plan, CO₂ savings estimate. Concise, warm, plain text.`;
    ...
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <Card className="coach-card">
        <div className="coach-messages" ref={scrollRef} role="log" aria-live="polite" aria-label="Conversation">
          ...
          {loading && <div className="coach-msg assistant" aria-live="assertive"><div className="coach-bubble"><Loader2 size={15} className="spin" aria-hidden="true" /> Thinking…</div></div>}
        </div>
        <div className="coach-quick">
          <button className="btn btn-ghost" onClick={() => send("Based on my data, give me a carbon impact summary, recommendations, monthly reduction plan, and CO₂ savings.")} disabled={loading} aria-label="Generate personalized plan">
        ...
        <div className="coach-input-row" role="form" aria-label="Send message">
          ...
          <input id="coach-input" ref={inputRef} placeholder="e.g. I drive to work daily and eat meat most nights..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} aria-label="Type your message" />
          <button className="btn btn-primary" onClick={handleSend} disabled={loading} aria-label="Send message"><Send size={15} aria-hidden="true" /></button>
        </div>
      </Card>
    </main>
  );
}
CoachPage.propTypes = { result: PropTypes.object.isRequired };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const content = [
        ...
        { type:"text", text:'Electricity bill. Find total kWh consumed and billing period months. Respond ONLY with JSON: {"units":<number>,"periodMonths":<number>,"summary":"<one sentence>"}' },
      ];
      ...
      setError("Couldn't read the bill clearly. Try a clearer photo or enter usage manually.");
    ...
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <Card>
        <div className="scanner-upload">
          <input id="bill-file" type="file" accept="image/*,application/pdf" onChange={onFile} style={{ display:"none" }} aria-label="Upload electricity bill" />
        ...
        {preview && <img src={preview} alt="Preview of uploaded electricity bill" className="scanner-preview" />}
        <div className="footer-row">
          <button className="btn btn-primary" onClick={analyze} disabled={!file||loading} aria-label="Analyse bill">
        ...
        {result && (
          <Card className="scanner-result" aria-live="polite">
            ...
            <div className="footer-row">
              <button className="btn btn-primary" onClick={applyToCalculator} aria-label="Apply extracted data to calculator">Apply to calculator <ArrowRight size={14} aria-hidden="true" /></button>
            </div>
          </Card>
        )}
      </Card>
...
ScannerPage.propTypes = { setInputs: PropTypes.func.isRequired };

/* ============================================================
   PREDICTIONS PAGE
   ============================================================ */
function PredictionsPage({ entries }) {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      {!pred
        ? <Card><EmptyState icon={TrendingUp} title="Not enough data yet" text="Save at least two calculations so we can detect a trend." /></Card>
        : <>
...
PredictionsPage.propTypes = { entries: PropTypes.array.isRequired };
                ...
                <li key={item.id}>
                  <Card className={`roadmap-item ${done?"done":""}`}>
                    ...
                    <div className="roadmap-body">
                      ...
                      <p>{item.desc}</p>
                      <p className="roadmap-saving">Estimated saving: {item.saving.toFixed(2)} t CO₂e/year</p>
                    </div>
                    <button className={`btn ${done?"btn-primary":"btn-ghost"}`} onClick={()=>toggleRoadmapItem(item.id)} aria-label={done?`${item.title} — done`:`Mark "${item.title}" as done`} aria-pressed={done}>
...
RoadmapPage.propTypes = {
  inputs:           PropTypes.object.isRequired,
  result:           PropTypes.object.isRequired,
  roadmapDone:      PropTypes.array.isRequired,
  toggleRoadmapItem:PropTypes.func.isRequired,
};
...
function OffsetPage({ result, treesPlanted, setTreesPlanted }) {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
      <PageHeader icon={TreePine} title="Tree Offset Calculator" subtitle={`Each tree absorbs roughly ${TREE_ABSORPTION} kg CO₂ per year.`} />
      <section aria-label="Offset statistics">
        <div className="stat-grid">
          <Card className="stat-card"><div className="stat-icon" style={{background:"linear-gradient(135deg,#34D399,#A7F3D0)"}} aria-hidden="true"><TreePine size={18}/></div><div><p className="stat-value">{treesNeeded}</p><p className="stat-label">Trees needed/year</p></div></Card>
          <Card className="stat-card"><div className="stat-icon" style={{background:"linear-gradient(135deg,#22D3EE,#38BDF8)"}} aria-hidden="true"><CheckCircle2 size={18}/></div><div><p className="stat-value">{treesPlanted}</p><p className="stat-label">Trees logged</p></div></Card>
        </div>
      </section>
      <Card style={{ marginTop:20 }}>
        ...
        <p className="hint-text" style={{ marginTop:8 }} aria-live="polite">{treesPlanted>=treesNeeded?"You've fully offset your annual footprint — great work!":`${treesNeeded-treesPlanted} more trees to fully offset this year.`}</p>
        <div className="goal-input-row" role="form" aria-label="Log planted trees">
          ...
          <input id="trees-add" type="number" min="0" placeholder="Trees planted this month" value={add} onChange={(e)=>setAdd(e.target.value)} className="form-input" />
          <button className="btn btn-primary" onClick={()=>{const n=parseInt(add,10);if(!isNaN(n)&&n>0){setTreesPlanted(p=>p+n);setAdd("");}}} aria-label="Log planted trees"><Plus size={15} aria-hidden="true"/> Log trees</button>
        </div>
      </Card>
      <Card style={{ marginTop:20 }}>
        <h2 className="card-title">Visualizing your offset need</h2>
        <div className="tree-grid" role="img" aria-label={`${Math.min(treesNeeded,maxDisplay)} tree icons: ${treesPlanted} planted, ${Math.max(0,Math.min(treesNeeded,maxDisplay)-treesPlanted)} still needed`}>
...
OffsetPage.propTypes = {
  result:          PropTypes.object.isRequired,
  treesPlanted:    PropTypes.number.isRequired,
  setTreesPlanted: PropTypes.func.isRequired,
};
...
function GamificationPage({ gami, setGami, sustainabilityScore }) {
  ...
  const toggleMission = useCallback((id, xpVal) => {
    setGami((g) => {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <section aria-label="Level and score" className="grid-2">
        <Card><h2 className="card-title">Your level</h2><div className="gauge-card"><RadialScore value={xpIntoLevel} max={XP_PER_LEVEL} label={`Level ${level}`} sublabel={`${xpIntoLevel}/${XP_PER_LEVEL} XP`}/></div></Card>
        <Card><h2 className="card-title">Sustainability score</h2><div className="gauge-card"><RadialScore value={sustainabilityScore} max={100} label="Score" sublabel="out of 100"/></div></Card>
      </section>
      <Card style={{ marginTop:20 }}>
          ...
          {DAILY_MISSIONS.map((m) => {
            ...
            return (
              <li key={m.id} className="mission-row">
                <button className={`mission-check ${done?"checked":""}`} onClick={()=>toggleMission(m.id,m.xp)} disabled={done} aria-label={done?`${m.label} — completed`:`Mark as complete (+${m.xp} XP)`} aria-pressed={done}>{done&&<CheckCircle2 size={14} aria-hidden="true"/>}</button>
...
GamificationPage.propTypes = {
  gami:               PropTypes.object.isRequired,
  setGami:            PropTypes.func.isRequired,
    ...
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <Card style={{ marginBottom:20 }}>
        ...
        <div className="goal-input-row" role="form" aria-label="Join leaderboard">
          <label htmlFor="leaderboard-name" className="visually-hidden">Display name</label>
          <input id="leaderboard-name" placeholder="Your display name" value={name} onChange={e=>setName(e.target.value)} className="form-input" style={{width:"auto",flex:1}}/>
          <button className="btn btn-primary" onClick={join} aria-label={joinedLeaderboard?"Update score":"Join leaderboard"}>{joinedLeaderboard?"Update my score":"Join"}</button>
        </div>
      ...
      <Card>
        ...
        {loading
          ? <div className="skeleton-list" aria-busy="true" aria-label="Loading">{[1,2,3].map(i=><div key={i} className="skeleton-row"/>)}</div>
          : board.length===0
            ? <EmptyState icon={Award} title="No entries yet" text="Be the first to join the community leaderboard."/>
            : <ol className="leaderboard-list" aria-label="Rankings">
                {board.map((e,i)=>(
                  <li key={e.name} className={`leaderboard-row ${e.name===profile.name?"me":""}`} aria-label={`Rank ${i+1}: ${e.name}, score ${e.score}${e.name===profile.name?" (you)":""}`}>
...
LeaderboardPage.propTypes = {
  ...
  sustainabilityScore: PropTypes.number.isRequired,
  result:              PropTypes.object.isRequired,
  joinedLeaderboard:   PropTypes.bool.isRequired,
  setJoinedLeaderboard:PropTypes.func.isRequired,
};
    ...
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <Card style={{ marginBottom:20 }}>
        ...
        <ul className="tip-grid" style={{listStyle:"none",padding:0}}>
          {ECO_TIPS.map((t) => {
            const icons = { "Switch to LED bulbs":Lightbulb, "Carpool or share rides":Users, "Unplug standby devices":Zap, "Compost kitchen waste":Recycle, "Air-dry laundry":Wind, "Buy local & seasonal produce":Leaf, "Fix leaking taps promptly":Droplets, "Choose repair over replace":ShoppingBag };
            const Icon = icons[t.title] || Leaf;
            return (
      ...
      <Card>
        ...
        <ul className="product-list" style={{listStyle:"none",padding:0}}>
          {GREEN_PRODUCTS.map(p=>(
            <li key={p.name} className="product-row"><p className="tip-title">{p.name}</p><p className="tip-detail">{p.note}</p></li>
          ))}
        </ul>
      </Card>
    </main>
...
/* ============================================================
   ANALYTICS PAGE
   ============================================================ */
function AnalyticsPage({ entries }) {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <Card style={{ marginBottom:20 }}>
        ...
        <div className="footer-row">
          <button className="btn btn-primary" onClick={downloadCSV} disabled={entries.length===0} aria-label="Download CSV report"><Download size={15} aria-hidden="true"/> Download CSV</button>
          <button className="btn btn-ghost" onClick={()=>window.print()} aria-label="Print report"><BarChart3 size={15} aria-hidden="true"/> Print report</button>
        </div>
      </Card>
      <Card style={{ marginBottom:20 }}>
        <h2 className="card-title">Historical comparison</h2>
        {entries.length===0
          ? <EmptyState icon={BarChart3} title="No history yet" text="Save calculations to build your analytics."/>
          : <ResponsiveContainer width="100%" height={220}>
      ...
      <Card>
        ...
        {entries.length===0
          ? <p className="hint-text">No entries saved yet.</p>
...
AnalyticsPage.propTypes = { entries: PropTypes.array.isRequired };
...
function ProfilePage({ profile, setProfile, gami, sustainabilityScore, goal, entries, result }) {
  ...
  return (
    <main className="page" id="main-content" aria-labelledby="page-title">
      ...
      <section aria-label="Profile" className="grid-2">
        <Card>
          <h2 className="card-title">Display name</h2>
          <div className="goal-input-row" role="form" aria-label="Update name">
            <label htmlFor="profile-name" className="visually-hidden">Display name</label>
            <input id="profile-name" placeholder="Your name" value={nameInput} onChange={e=>setNameInput(e.target.value)} className="form-input" style={{width:"auto",flex:1}}/>
            <button className="btn btn-primary" onClick={()=>setProfile(p=>({...p,name:nameInput.trim()}))} aria-label="Save name">Save</button>
          </div>
        ...
        <Card>
          ...
          {goal
            ? <><p className="hint-text">Set on {goal.date}: reduce from {goal.baseline?.toFixed(2)} t to {goal.target?.toFixed(2)} t.</p><ProgressBar value={Math.max(0,(goal.baseline||0)-result.totalTons)} max={Math.max(0.01,(goal.baseline||0)-(goal.target||0))} label="Goal progress"/></>
      ...
      <Card style={{ marginTop:20 }}>
        ...
        {entries.length===0
          ? <p className="hint-text">Nothing saved yet.</p>
          : <ol className="leaderboard-list" aria-label="Recent entries">
              {entries.slice(-5).reverse().map(e=>(
                <li key={e.date} className="leaderboard-row">
                  <span className="leaderboard-name">{e.date}</span>
                  <span className="leaderboard-score">{e.total.toFixed(2)} t CO₂e</span>
                </li>
              ))}
            </ol>
        }
...
export default function App() {
  ...
  const safeGet = useCallback(async (key) => {
  ...
  const notifications = useMemo(() => {
    const list = [];
    if (result.totalTons>PARIS_TARGET) list.push(`Footprint is ${(result.totalTons-PARIS_TARGET).toFixed(1)} t above the 1.5°C target — check your Roadmap.`);
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "greenhub":     return <GreenHubPage/>;
  ...
  return (
    <div className={`app ${theme==="light"?"light":""}`}>
      ...
      <nav className="sidebar" aria-label="Main navigation">
        ...
        <ul style={{listStyle:"none",padding:0,margin:0,flex:1}}>
          {NAV_ITEMS.map((item)=>{
            ...
            return (
              <li key={item.id}>
                <button className={`sidebar-link ${page===item.id?"active":""}`} onClick={()=>setPage(item.id)} aria-current={page===item.id?"page":undefined} aria-label={item.label}>
      ...
      <header className="topbar" role="banner">
        <div className="sidebar-logo"><Sparkles size={18} aria-hidden="true"/> <span>EcoSense AI</span></div>
        <div className="topbar-actions">
          ...
          <button className="icon-btn" onClick={()=>setMobileMenuOpen(o=>!o)} aria-label="Open menu" aria-expanded={mobileMenuOpen}><Menu size={18} aria-hidden="true"/></button>
        </div>
      </header>
      ...
      <nav className="bottom-nav" aria-label="Primary navigation">
        ...
        <button className={`bottom-nav-item ${mobileMenuOpen?"active":""}`} onClick={()=>setMobileMenuOpen(o=>!o)} aria-label="More options" aria-expanded={mobileMenuOpen}>
...
.app{--bg:#070D14;--bg-glow-1:rgba(52,211,153,0.12);--bg-glow-2:rgba(56,189,248,0.10);--surface:rgba(255,255,255,0.045);--surface-solid:#111B2E;--surface-border:rgba(255,255,255,0.09);--text:#EAF2F0;--text-muted:#8FA3A0;--accent-grad:linear-gradient(135deg,#34D399,#38BDF8);--danger:#F87171;--warning:#FBBF24;font-family:'Inter',sans-serif;color:var(--text);background:var(--bg);background-image:radial-gradient(circle at 10% 10%,var(--bg-glow-1),transparent 40%),radial-gradient(circle at 90% 20%,var(--bg-glow-2),transparent 40%);min-height:100vh;display:grid;grid-template-columns:240px 1fr;position:relative;}
.app.light{--bg:#F1F7F5;--surface:rgba(255,255,255,0.75);--surface-solid:#FFFFFF;--surface-border:rgba(15,40,35,0.08);--text:#0E211C;--text-muted:#5B6E68;}
...
@media(max-width:980px){.app{grid-template-columns:1fr;}.sidebar{display:none;}.topbar{display:flex;}.bottom-nav{display:flex;}.main{padding:16px 14px 90px;}.stat-grid{grid-template-columns:repeat(2,1fr);}.grid-2{grid-template-columns:1fr;}.history-row{grid-template-columns:80px 60px repeat(6,60px);overflow-x:auto;}.history-table{overflow-x:auto;}}
